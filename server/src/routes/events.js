...
router.get('/', async (req, res) => {
  try {
    ...
    const result = events.map(event => ({
      ...
      hasWheelchairAccess: event.has_wheelchair_access,
      hasAsl: event.has_asl,
      hasSensoryFriendly: event.has_sensory_friendly,
      hasAdaParking: event.has_ada_parking,
...
router.get('/venue-boundaries', async (req, res) => {
  try {
    ...
    const { rows: venueRows } = await pool.query(
      `SELECT id, name AS venue_name, boundary_coordinates AS coordinates, city, latitude, longitude, aliases
       FROM venues
       WHERE city = $1
       ORDER BY name ASC`,
      [city]
